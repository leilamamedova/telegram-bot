...
const bot = new Telegraf('7014352358:AAGbQb-R82QLBa7NWNCzb1k3jwXZS_wF56w');
...
bot.action('flip_a_coin', async(ctx) => {
    await ctx.editMessageText(`${getCoinSide()}\nEdited: ${getCurrentDateTime()}`, coinInlineKeyboard);
});
...
bot.action('random_number', async(ctx) => {
    await ctx.editMessageText(`${getRandomNumber()}\nEdited: ${getCurrentDateTime()}`, numberInlineKeyboard);
});
